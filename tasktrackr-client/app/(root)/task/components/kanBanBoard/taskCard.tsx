...
function TaskCard({
  ...
  const [deleteRes, isDeleteResLoading, deleteResRequest] = useAxios<
    ITask,
    null
  >(DELETE_TASK_BY_ID_URL + deleteData?._id, "DELETE");
  ...
  const handelEditOpen = (data: ITask) => {
    setOpen(true);
    setEditData(data);
  };
  ...
  return (
    <>
      ...
      <div
        ...
        {...attributes}
        style={style}
        className={`mb-2 ${isDragging ? "shadow-2xl scale-105" : ""}`}
      >
          ...
          <CardContent className="p-4 space-y-2">
            ...
            <div className="flex items-end justify-between mt-4">
              <div className="flex gap-1 items-center">
                <span
                  className={`text-xs text-[${task.priority_details.color}] capitalize`}
                >
                  {task.priority_details.name}
                </span>
                <FlagTriangleRight
                  className={`h-5 w-5 text-[${task.priority_details.color}]`}
                />
              </div>
              <div className="flex gap-1 items-center">
                <span
                  className={`text-xs text-[${task.category_details.color}] capitalize`}
                >
                  {task.category_details.name}
                </span>
                <FlagTriangleRight
                  className={`h-5 w-5 text-[${task.category_details.color}]`}
                />
              </div>
            </div>
          </CardContent>
